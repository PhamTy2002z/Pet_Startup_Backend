...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) =>
    file.mimetype.startsWith('image/') ? cb(null, true) : cb(new Error('Only image files')),
}).single('image');
...
exports.createTheme = async (req, res) => {
  const { name, description = '', isActive = true, order = 0, isPremium = false, price = 0, inStore = true } = req.body;
  if (!name || !req.file) return res.status(400).json({ error: 'name & image required' });
...
exports.getAllThemes   = async (_req, res) => res.json(await Theme.find().sort({ order: 1, name: 1 }));
...
exports.batchUpdateThemeStatus = async (req, res) => {
  const ops = (req.body.themes || []).filter((t) => t.id).map((t) => ({
    updateOne: { filter: { _id: t.id }, update: { isActive: !!t.isActive } },
  }));
...
exports.getStoreThemes = async (_req, res) =>
  res.json(await Theme.find({ isActive: true, inStore: true }).sort({ isPremium: 1, order: 1, name: 1 }));
...
exports.getPurchasedThemes = async (req, res) => {
  ...
  const list = await UserTheme.find({ petId: req.params.petId }).populate('themeId');
  res.json(list.filter((ut) => ut.themeId?.isActive).map((ut) => ut.themeId));
};

/* ---------- Purchase ---------- */
exports.purchaseTheme = async (req, res) => {
  const { petId, themeId } = req.body;
  if (!petId || !themeId) return res.status(400).json({ error: 'petId & themeId required' });
  ...
  await session.withTransaction(async () => {
    ...
    if (!theme.isActive || !theme.inStore) throw new Error('Theme not for sale');

    const existed = await UserTheme.findOne({ petId, themeId }).session(session);
...
exports.applyPurchasedTheme = async (req, res) => {
  ...
  if (themeId) {
    ...
    if (!theme || !theme.isActive) return res.status(400).json({ error: 'Theme invalid' });
    if (theme.isPremium && !(await UserTheme.exists({ petId, themeId })))
      return res.status(403).json({ error: 'Theme not purchased' });
    pet.themeId = themeId;
  } else {
    pet.themeId = null; // remove theme
  }
  await pet.save();
  res.json(await Pet.findById(petId).populate('themeId'));
};

/* ---------- Active (free+purchased) ---------- */
exports.getActiveThemes = async (req, res) => {
  const { petId } = req.query;
  const free = await Theme.find({ isActive: true, isPremium: false }).lean();

  if (!petId) return res.json(free);

  const purchased = await UserTheme.find({ petId }).populate('themeId');
  const all = [...free, ...purchased.map((u) => u.themeId).filter(Boolean)];
  const uniq = Array.from(new Map(all.map((t) => [t._id.toString(), t])).values())
